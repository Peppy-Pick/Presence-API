    ...
    def get(self):
        ...
        try:
            ...
            if employee_id:
                ...
                # Fetch employee details from Firestore to include name
                ...
                if employee_doc.exists:
                    ...
                    for record in records:
                        record["employee_name"] = employee_data.get("name", "Unknown")
            ...
            # If no employee_id is provided, get records for all employees on that date
                ...
                # Get unique employee IDs from the records
                employee_ids = list(set(record.get("employee_id") for record in records if "employee_id" in record))
                
                # Create a mapping of employee_id to employee_name
                employee_names = {}
                for emp_id in employee_ids:
                    employee_doc = db.collection("employees").document(emp_id).get()
                    if employee_doc.exists:
                        employee_data = employee_doc.to_dict()
                        employee_names[emp_id] = employee_data.get("name", "Unknown")
                
                # Add employee names to each record
                for record in records:
                    emp_id = record.get("employee_id")
                    if emp_id in employee_names:
                        record["employee_name"] = employee_names[emp_id]
                    else:
...
class EmployeeAttendanceAPI(Resource):
    def get(self, employee_id):
        ...
        try:
            ...
            # Fetch employee details to include name
            employee_doc = db.collection("employees").document(employee_id).get()
...
class AttendanceLogsAPI(Resource):
    def get(self):
        ...
        try:
            ...
            # Get unique employee IDs from the logs
            employee_ids = list(set(log.get("employee_id") for log in logs if "employee_id" in log))
            ...
            # Add employee names to each log
            for log in logs:
                emp_id = log.get("employee_id")
                if emp_id in employee_names:
                    log["employee_name"] = employee_names[emp_id]
                else:
